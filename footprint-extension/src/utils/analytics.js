...
export function calculateProductivityScore(sessions, userCategories = {}) {
  ...
  sessions.forEach(session => {
    ...
    if (userCategories.productive && userCategories.productive.includes(session.url)) {
      productiveTime += duration;
    }
  });
...
export function detectContentBubbles(topicAnalyses) {
  if (!topicAnalyses || topicAnalyses.length === 0) return { diversity: 0, topTopics: [] };
  ...
  const diversity = uniqueTopics > 0 ? Math.round((uniqueTopics / Math.max(totalTopics / uniqueTopics, 1)) * 100) : 0;
